...
class Board:
    ...
    def achar_lc_peca(self, peca: Peca, matriz: np.ndarray) -> tuple[int, int] | None:
        """
        ...
        for li in range(8):
            for co in range(8):
                if matriz[li, co] == peca:
                    return (li, co)
        return None


    def achar_lc_rei(self, cor: Cor, matriz: np.ndarray) -> tuple[int, int] | None:
        """
        Encontra a linha e coluna de uma instância de peça na matriz.
